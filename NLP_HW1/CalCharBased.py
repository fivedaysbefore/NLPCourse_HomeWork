...
def CalUnigram(corpus,count):
    ...
    for line in corpus:
        for x in line:
            ...
            words_len += 1
        GetUnigramTF(words_tf, split_words)
        split_words = []
        line_count += 1

    print("语料库字数:", count)
    print("分词个数:", words_len)
    print("平均词长:", round(count / words_len, 2))
    entropy = []
    for uni_word in words_tf.items():
        entropy.append(-(uni_word[1] / words_len) * math.log(uni_word[1] / words_len, 2))
    print("基于词的一元模型的中文信息熵为:", round(sum(entropy), 2), "比特/词")
    after = time.time()
    print("运行时间:", round(after - before, 2), "s")

def CalBigram(corpus, count):
    before = time.time()
    split_words = []
    words_len = 0
    line_count = 0
    ...
    for line in corpus:
        ...
        GetUnigramTF(words_tf, split_words)
        GetBigramTF(bigram_tf, split_words)
    ...
    print("基于词的二元模型的中文信息熵为:", round(sum(entropy), 2), "比特/词")

    after = time.time()
    print("运行时间:", round(after - before, 2), "s")

def CalTrigram(corpus,count):
    before = time.time()
    split_words = []
    words_len = 0
    line_count = 0
    ...
    for line in corpus:
        ...
        GetBigramTF(words_tf, split_words)
        GetTrigramTF(trigram_tf, split_words)
